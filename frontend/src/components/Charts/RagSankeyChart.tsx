...
export default function RagSankeyChart({ nodes, links }: RagSankeyChartProps) {
  ...
  const option: EChartsOption = {
    ...
    tooltip: {
      ...
      formatter: (params) => {
        const p = params as { dataType?: string; name?: string; data?: SankeyLink & { source?: string; target?: string } }
  ...
  return (
    <HudPanel className="chart-panel">
      <h3 className="chart-panel__title">RAG 知识链路（桑基图 · 真实对话）</h3>
