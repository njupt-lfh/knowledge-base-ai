...
import { Button, Input, Modal, message, Select, Space, Table, Tag, Tabs, Typography } from 'antd'
...
export default function GapTasks() {
  ...
  const onIngest = async (gap: KnowledgeGap, manual?: string) => {
    ...
    try {
      ...
      message.success(`已入库（${res.data.ingest_allowed} 块通过门禁）`)
      setManualModal(null)
      setManualContent('')
      fetchGaps()
    } catch (e: unknown) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: unknown, row: KnowledgeGap) => {
        ...
        return s?.content ? (
          <Typography.Text ellipsis>{s.title ? `${s.title}: ` : ''}{s.content}</Typography.Text>
        ) : (
      ...
      render: (_: unknown, row: KnowledgeGap) => (
        <Space wrap>
          {row.gap_type === 'KNOWLEDGE_ABSENT' && row.status === 'manual_required' && (
            <Button size="small" type="primary" onClick={() => { setManualModal(row); setManualContent('') }}>
              人工添加
            </Button>
          )}
          ...
          {row.status === 'pending' && row.gap_type === 'RETRIEVAL_MISS' && (
            <>
              <Button size="small" type="link" onClick={() => onStatus(row.id, 'approved')}>标记已处理</Button>
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <Modal
        ...
        onCancel={() => { setManualModal(null); setManualContent('') }}
        okText="入库"
      ...
      >
        ...
        <Input.TextArea rows={6} value={manualContent} onChange={(e) => setManualContent(e.target.value)} placeholder="输入要入库的知识正文…" />
      </Modal>
    </div>
  )
}
