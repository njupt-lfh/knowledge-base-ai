"""Chunk 级实体关系抽取（Phase 3，LLM / Mock 规则双模式）。

职责：
    从单个 chunk 文本抽取 subject-predicate-object 三元组，
    供 graph_store_service 写入 KgRelation。

在流水线中的位置：
    graph_store_service.sync_chunk_graph → extract_triples_from_chunk

依赖服务：
    - LLMService：非 Mock 模式下调用 LLM 结构化抽取
"""
...
async def extract_triples_from_chunk(content: str) -> list[dict[str, Any]]:
    ...
    返回:
        含 subject/predicate/object/confidence 的三元组列表
    """
