...
async def main() -> int:
    ...
    async with async_session() as db:
        db.add(KnowledgeBase(id=kb_id, name="p44", embedding_model="m", chunk_size=200, chunk_overlap=20))
        ...
        docs = (await db.execute(select(Document).where(Document.knowledge_base_id == kb_id))).scalars().all()
        assert len(docs) == 1 and docs[0].status == "completed"

        chunks = (
            await db.execute(select(Chunk).where(Chunk.document_id == docs[0].id))
        ).scalars().all()
