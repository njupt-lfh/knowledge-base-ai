...
async def main() -> int:
    ...
    async with async_session() as db:
        ...
        if "health" not in scan or "suggestions" not in scan:
            ...
            return 1
        print(f"  suggestions={len(scan['suggestions'])}, cold_90d={scan['health'].get('cold_count_90d')}")
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        res = await client.get(f"/api/knowledge-bases/{kb.id}/governance/suggestions", params={"scan_duplicates": False})
