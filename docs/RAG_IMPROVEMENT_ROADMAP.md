...
> **chunk 级 CP（~0.25–0.52）≠ RAGAS CP（~0.72–0.78）**。解读报告时须区分双轨口径，避免被误解为「数字矛盾」。
...
召回与命中率已在 0.85–0.90 档，说明覆盖足够；CP-chunk 低主要反映 **top-k 噪声** 与 **轻量重排语义不足**。已落地 Cross-Encoder、检索后过滤与动态 top_k，并规划结构分块，目标 chunk 级 **0.45–0.50**。

### RAGAS 答案相关性（AR）为什么偏低？
...
### multi_hop / Agent 效果如何？

Phase 3 显示盲目 SIM-RAG 可能伤 fact 题。路线图为 **仅 multi_hop 走图谱分路与精准 SIM-RAG**；v2 multi_hop 召回仍在提升中（当前约 0.59，长期目标 0.82；短期可接受 **0.6–0.7 + v1 回归 PASS**）。
